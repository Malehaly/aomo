...
namespace RTSEngine 
{
	public class NPCBuildingPlacement : MonoBehaviour {
		...
		void PlaceBuilding ()
		{
			int LayerID = LayerMask.NameToLayer ("Building"); //set the buildings layer:
			int LayerIDMinimap = LayerMask.NameToLayer ("MinimapQuad");
			PendingBuildings[0].BuildingToAdd.gameObject.layer = LayerID;
			if (PendingBuildings [0].BuildingToAdd.transform.childCount > 0) {
				//enable some of the buildings component (that the have the layer "Building")
				for (int i = 0; i < PendingBuildings [0].BuildingToAdd.transform.childCount; i++) {
					//if it's no the default ID.
					if (PendingBuildings [0].BuildingToAdd.transform.GetChild (i).gameObject.layer != 0) {
						PendingBuildings [0].BuildingToAdd.transform.GetChild (i).gameObject.layer = LayerID;
						PendingBuildings [0].BuildingToAdd.transform.GetChild (i).gameObject.gameObject.SetActive (true);
					}
				}
				//TODO doc change - minimapquad layer
				if (PendingBuildings [0].BuildingToAdd.transform.Find ("MinimapQuad").gameObject != null) {
					PendingBuildings [0].BuildingToAdd.transform.Find("MinimapQuad").gameObject.layer = LayerIDMinimap;
				}
			}
		...
		public void AddBuildingToList (Building BuildingToAdd, int CenterID, GameObject BuildAround)
		{
			...
			SpawnPos.y = GameMgr.SelectionMgr.TerrainObj.gameObject.GetComponent<Terrain>().SampleHeight(FactionMgr.BuildingCenters [CenterID].transform.position)+MapHeight; //make sure that spawn pos is the same as the map height.
			...
			//set the building's layer because we don't want the camera to show the faction placing the building. only show it when it's built.
			int LayerID = LayerMask.NameToLayer ("Hidden");
			BuildingClone.gameObject.layer = LayerID;
			if (BuildingClone.transform.childCount > 0) {
				for (int i = 0; i < BuildingClone.transform.childCount; i++) {
					//same goes to the building's components:
					if (BuildingClone.transform.GetChild (i).gameObject.layer != 0) {
						BuildingClone.transform.GetChild (i).gameObject.layer = LayerID;
						BuildingClone.transform.GetChild (i).gameObject.gameObject.SetActive (false);
		...
		public void ConstructBuilding (Building NeededBuilding)
		{
			if (NeededBuilding == null) {
				return;
			}
			...
			int Amount = NeededBuilding.MaxBuilders - NeededBuilding.CurrentBuilders.Count;

			//Search for this faction's idle builders:
			while (NeededBuilding.CurrentBuilders.Count < NeededBuilding.MaxBuilders && i < FactionMgr.Builders.Count) {
				if (FactionMgr.Builders [i] != null) {
					...
					if (FactionMgr.Builders [i].UnitMvt.IsIdle() == true) {
						...
						FactionMgr.Builders [i].SetTargetBuilding (NeededBuilding);
			...
			Amount -= GetInProgressBuilders (NeededBuilding); //using this method, we will remove the amount of the in progress builders that are getting created and directly send to this building.
			...
			if (Amount > 0) {
				//Create some.
				CanProduceBuilders = CreateBuilders (NeededBuilding, NeededBuilding.MaxBuilders, NeededBuilding.transform.position);
			}
			...
			if (Amount == 0 || CanProduceBuilders == false) {
				...
				if (SentExistingBuilders == false) {
					...
					SendCollectorsToBuild (NeededBuilding);
				}
			}
		}
		...
		public void SendCollectorsToBuild (Building TargetBuilding)
		{
					...
					if (CurrentBorder.ResourcesInRange [j] != null) {
						if (CurrentBorder.ResourcesInRange [j].gameObject.activeInHierarchy == true) { //if the resource is active
							if (CurrentBorder.ResourcesInRange [j].FactionID == FactionID) { //If this resource is being collected by this faction:
								if (CurrentBorder.ResourcesInRange [j].CurrentCollectors.Count > 0) { //If this resource has collectors.
									if (CurrentBorder.ResourcesInRange [j].CurrentCollectors [0].BuilderMgr) { //If at least one of the collectors has a builder component.
			...
			if (TargetResource != null) {
				int FreeAmount = Mathf.RoundToInt (TargetResource.CurrentCollectors.Count / 2);
				if (FreeAmount < 1)
					FreeAmount = 1;

				int MaxAmount = FreeAmount;
				int n =0;

				//Loop through all the collectors of the target resource:
				while (n < MaxAmount && FreeAmount > 0) {
					if (TargetResource.CurrentCollectors[n] != null) {
						//Make sure the collector can also build stuff:
						if (TargetResource.CurrentCollectors [n].BuilderMgr) {
							Unit CurrentUnit = TargetResource.CurrentCollectors [n];
