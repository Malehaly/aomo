...
namespace RTSEngine 
{
	public class NPCArmy : MonoBehaviour {
		...
		public Vector2 DefaultAttackRange = new Vector2(8.0f,10.0f); //The default attacking range for units when they are not attacking nor defending.
		...
		public Vector2 ArmyHealersRatioRange = new Vector2(0.6f, 0.8f);
		float ArmyHealersRatio;
		public Vector2 ArmyConvertersRatioRange = new Vector2 (0.6f, 0.8f);
		float ArmyConvertersRatio;
		...
		void Awake () {
			...
			ArmyHealersRatio = Random.Range (ArmyHealersRatioRange.x, ArmyHealersRatioRange.y);
			ArmyConvertersRatio = Random.Range (ArmyConvertersRatioRange.x, ArmyConvertersRatioRange.y);
				...
				if (TargetFaction != null) {
					if (AttackTimer > 0) {
						...
						if (TargetBuilding == null) {
							AttackTimer = -1.0f;
						} else if (TargetingBuilders == false && TargetBuilding.CurrentBuilders.Count > 0) {
		...
		public void SendUnitsToTargetBuilding ()
		{
			if (TargetBuilding != null) { //if there's a target building
				...
				if (TargetBuilding.CurrentBuilders.Count > 0) {
					int n = 0;
					//attack these builders instead.
					while (n < TargetBuilding.CurrentBuilders.Count) {
						if (TargetBuilding.CurrentBuilders [n].IsBuilding == true) {
							Target = TargetBuilding.CurrentBuilders [0].gameObject;
				...
				if (Army.Count > 0) {
					for (int i = 0; i < Army.Count; i++) {
						...
						if (Army [i] != null) {
							...
							if (Army [i].AttackMgr.AttackTarget == null) { //set the attacking support range that will call nearby units for support.
								Army [i].AttackMgr.AttackRange = AttackingSupportRange;
							}
						}
					}
					SelectionMgr.LaunchAttack (Army, Target.gameObject, TargetingBuilders); //launch the attack.

					//move healers and converters to the target building
					//Healers:
					int HealersAmount = Mathf.RoundToInt(FactionMgr.Healers.Count*ArmyHealersRatio);
					int ConvertersAmount = Mathf.RoundToInt(FactionMgr.Converters.Count * ArmyConvertersRatio);
					...
					while (HealersAmount > 0 && j < FactionMgr.Healers.Count) {
						if (FactionMgr.Healers [j].TargetUnit == null) { //only move the unit if it has no target
							FactionMgr.Healers [j].UnitMvt.CheckUnitPath (Vector3.zero, Target.gameObject, GameMgr.MvtStoppingDistance, j, true);
						}
						HealersAmount--;
					...
					while (ConvertersAmount > 0 && j < FactionMgr.Converters.Count) {
						if (FactionMgr.Converters [j].TargetUnit == null) {
							FactionMgr.Converters [j].UnitMvt.CheckUnitPath (Vector3.zero, Target.gameObject, GameMgr.MvtStoppingDistance, j, true);
						}
						ConvertersAmount--;
			...
			if (Army.Count > 0) {
				for (int i = 0; i < Army.Count; i++) {
					if (Army [i] != null) {
						...
						Army [i].AttackMgr.AttackRange = Random.Range(DefaultAttackRange.x,DefaultAttackRange.y);
		...
		public void SendBackArmy ()
		{
			...
			for (int j = 0; j < AllUnits.Count; j++) {
				if (AllUnits[j] != null) {
					...
					AllUnits[j].CheckUnitPathLocal (Vector3.zero, GotoBuilding.gameObject, GameMgr.MvtStoppingDistance, -1); //send the units to the builindg that created them.
		...
		public void SetDefenseCenter (Building Center, int SourceFactionID)
		{
			...
			foreach (Collider Obj in ObjsInBorder) {
				...
				if (AttackUnit != null) {
                    ...
                    if (AttackUnit.Attacker == Attack.Attackers.Unit)
                    {
