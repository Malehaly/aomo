					...
					if (Units [i].StartCreatingAfter > 0) {
						...
						if (Units [i].SpawnTimer > 0) {
							...
							if (Units [i].AllCreated == false) { //still need to reach our goal:
								if ((GameMgr.Factions [FactionID].MaxPopulation - GameMgr.Factions [FactionID].CurrentPopulation) > 0) { //if we still have space in the population slots:
									...
									while (AmountLeft > 0 && j < Units [i].SourceBuildings.Count) { //go through the buildings that can create this unit:
										while (AmountLeft > 0 && Units [i].SourceBuildings [j].TasksQueue.Count < Units [i].SourceBuildings [j].MaxTasks) {//if this still has space for one more task and we still haven't finished creating all units
