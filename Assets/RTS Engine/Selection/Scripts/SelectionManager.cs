...
namespace RTSEngine
{
	public class SelectionManager : MonoBehaviour {
		...
		void Awake () 
		{
			...
			if (EnableSelectionGroups == true) { //if there's selection groups:
				//initialize them:
		...
		void Update () 
		{
			...
			/***********************************************************************************************************************************************/
			//Selection Groups:
			if (EnableSelectionGroups == true) { //if enabled
				int NumKey = GetNumKey();
				if (NumKey != -1) { //if key is pressed
		...
		public void MoveSelectedUnits (Vector3 Destination)
		{
			if (SelectedUnits.Count > 0) { //make sure, at least a unit has been selected
				if (GameManager.PlayerFactionID == SelectedUnits [0].FactionID) { //Units from the player team can be moved by the player, others can't.
					...
					//mvt order:
					AudioManager.PlayAudio (GameMgr.GeneralAudioSource.gameObject, SelectedUnits [0].MvtOrderAudio, false);
			...
			if (SelectedUnits.Count > 0) {
				for (int i = 0; i < SelectedUnits.Count; i++) {
					if (SelectedUnits [i] != null) {
                        ...
                        {
                            Continue = false; //if the unit doesn't have an attack manager at all then don't continue as well
                        }
		...
		public void LaunchAttack (List<Unit> SelectedUnits, GameObject TargetObj, bool ChangeTarget)
		{
			...
			if (SelectedUnitTypes.Count > 0) {
				...
				for (int j = 0; j < SelectedUnitTypes.Count; j++) {
					if (SelectedUnitTypes [j].UnitsList [0].AttackMgr) {
                        ...
                        MainAttackUnit = CurrentUnitList[0]; //this unit will play the attack order audio clip
							...
							if (CurrentUnitList [i] != null) {
								...
								if (CurrentUnitList [i].IsInvisible == false || (CurrentUnitList [i].IsInvisible == true && CurrentUnitList [i].InvisibilityMgr.CanAttack == true)) {
									if ((ChangeTarget == false && CurrentUnitList [i].AttackMgr.AttackTarget == null) || ChangeTarget == true) {
										...
										if (AttackManager.Instance.CanAttackTarget (CurrentUnitList[i].AttackMgr, TargetObj, CurrentUnitList [i].AttackMgr.AttackCategoriesList) == true) {
